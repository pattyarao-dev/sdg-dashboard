...
export default function IndicatorDashboard({
  ...
  const getLineChartOptions = (indicatorName: string, annualData: any) => {
    ...
    const categories = annualData.annualProgress.map((item: any) =>
      item.year.toString(),
    );
  ...
  const exportToPDF = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/export_indicator_pdf`,
        {
          method: "POST",
          headers: {
    ...
    return indicators.flatMap((indicator) => {
      ...
      const goalSubIndicators: FlattenedIndicatorItem[] =
        indicator.td_goal_sub_indicator.map((goalSubIndicator) => ({
          ...
          goalIndicatorId: indicator.goal_indicator_id,
          goalSubIndicatorId:
            goalSubIndicator.md_sub_indicator.sub_indicator_id,
      ...
      const goalSubIndicatorIds = new Set(
        goalSubIndicators.map((item) => item.id),
      );
      ...
      return [
        mainIndicator,
        ...goalSubIndicators,
        ...uniqueHierarchicalSubIndicators,
      ];
    });
  }, [indicators]);
  ...
  const fetchAnnualProgressData = async () => {
    try {
      ...
      const itemsWithGoalTargets = allIndicatorsAndSubIndicators.filter(
        (item) => item.hasGoalTarget,
      );
      ...
      const indicatorAnnualPromises = itemsWithGoalTargets
        ...
        .map(async (item) => {
          try {
            ...
            console.error(
              `Error fetching annual progress for ${item.goalIndicatorId}:`,
              error,
            );
            return {
              key: `indicator-${item.goalIndicatorId}`,
              data: { hasAnnualData: false },
            };
          }
        });
      ...
      const subIndicatorAnnualPromises = itemsWithGoalTargets
        ...
        .map(async (item) => {
          try {
            ...
            console.error(
              `Error fetching annual progress for ${item.goalSubIndicatorId}:`,
              error,
            );
            return {
              key: `sub-indicator-${item.goalSubIndicatorId}`,
        ...
        const indicatorProgressPromises = itemsWithGoalTargets
          ...
          .map(async (item) => {
            try {
              ...
              console.error(
                `Error fetching indicator progress for ${item.goalIndicatorId}:`,
                error,
              );
              return {
                key: `indicator-${item.goalIndicatorId}`,
        ...
        const subIndicatorProgressPromises = itemsWithGoalTargets
          ...
          .map(async (item) => {
            try {
              ...
              console.error(
                `Error fetching sub-indicator progress for ${item.goalSubIndicatorId}:`,
                error,
              );
              return {
                key: `sub-indicator-${item.goalSubIndicatorId}`,
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 sm:justify-between sm:items-center">
        <div>
          <h2 className="text-2xl font-bold" style={{ color: goalColor }}>
            Goal {goaldId} Indicators & Sub-indicators
          </h2>
          <p className="text-gray-600 text-sm mt-1">
            Track progress across{" "}
            {
              allIndicatorsAndSubIndicators.filter(
        ...
        {allIndicatorsAndSubIndicators.map((item, index) => {
          ...
          if (item.hasGoalTarget) {
            if (item.type === "indicator") {
              ...
              item.type === "sub-indicator" &&
              item.goalSubIndicatorId
            ) {
          ...
          const opacity =
            item.type === "indicator" ? 1 : Math.max(0.5, 1 - item.level * 0.1);
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {hasProgressData && annualData?.hasAnnualData && (
                        <HiChartBar
                          ...
                          onClick={() =>
                            openChartModal(annualData, item.name, item.type)
                          }
                    ...
                    {hasProgressData && annualData?.hasAnnualData && (
                      <p className="text-xs text-blue-600 mt-1">
                        📊 {annualData.totalYears} years of trend data (
                        {annualData.yearRange.start}-{annualData.yearRange.end})
                      </p>
                    )}
                  ...
                  {selectedModalData.annualData?.hasAnnualData ? (
                      ...
                      <div className="mb-4">
                        <Chart
                          options={getLineChartOptions(
                            selectedModalData.indicatorName,
                            selectedModalData.annualData,
                          )}
                          series={[
                            {
                              name: "Progress (%)",
                              data: selectedModalData.annualData.annualProgress.map(
                                (item: any) => item.progressPercentage,
                              ),
                            },
                          ]}
                          type="line"
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          <div className="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p className="text-lg font-bold text-gray-800">
                              {selectedModalData.annualData.totalYears}
                            </p>
                          </div>
                          <div className="bg-gray-50 p-3 rounded-lg">
                            <p className="text-xs text-gray-500 uppercase tracking-wide">
                            ...
                            <p className="text-lg font-bold text-gray-800">
                              {selectedModalData.annualData.yearRange.start}-
                              {selectedModalData.annualData.yearRange.end}
                            </p>
                          </div>
                          <div className="bg-gray-50 p-3 rounded-lg">
                            <p className="text-xs text-gray-500 uppercase tracking-wide">
                            ...
                            <p
                              ...
                              style={{ color: goalColor }}
                            >
                              {selectedModalData.annualData.annualProgress[
                                selectedModalData.annualData.annualProgress
                                  .length - 1
                          ...
                          <div className="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p className="text-lg font-bold text-green-600">
                              {(() => {
                                const first =
                                  selectedModalData.annualData.annualProgress[0]
                        ...
                        <div className="overflow-x-auto">
                          <table className="min-w-full bg-white border border-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200">
                              {selectedModalData.annualData.annualProgress.map(
                                (yearData: any, idx: number) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-2 text-sm text-gray-600">
                                      {yearData.value.toFixed(2)}
                                    </td>
                                    <td className="px-4 py-2 text-sm text-gray-600">
                                      {yearData.targetValue.toFixed(2)}
                                    </td>
                                    <td
                    ...
                    <div className="text-center py-12">
                      ...
                      <h4 className="text-lg font-semibold text-gray-600 mb-2">
                        No Annual Historical Data
                      </h4>
                      <p className="text-gray-500">
                        This {selectedModalData.indicatorType} does not have
